...
public interface EasyRAGAssistant {
    ...
    /**
     * @param message 用户消息
     * @param userId  多租户标识，会作为调用参数传入 {@code Query.metadata().invocationParameters()}
     *                供 {@code EasyRAGContentRetriever.dynamicFilter} 读取，实现租户隔离
     */
    String chat(@UserMessage String message, @V("userId") String userId);
}
