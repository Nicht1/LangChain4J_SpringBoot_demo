...
@Service
public class EasyRAGChatService {

    private final EasyRAGAssistantFactory assistantFactory;

    private final DatabaseChatMemoryStore memoryStore;

    private final ChatSessionService chatSessionService;

    private final Map<String, EasyRAGAssistant> sessionAssistants = new HashMap<>();



    @Autowired
    public EasyRAGChatService(EasyRAGAssistantFactory assistantFactory, DatabaseChatMemoryStore memoryStore, ChatSessionService chatSessionService) {
    ...
    public MessageResponseVO sendMessage(String sessionId, Long userId, String message) {
        ...
        // 获取或创建会话特定的助手
        EasyRAGAssistant easyRAGAssistant = this.getOrCreateAssistant(sessionId);


        // 生成回复（传入 userId 供 dynamicFilter 做多租户过滤）
        String response = easyRAGAssistant.chat(message, String.valueOf(userId));
