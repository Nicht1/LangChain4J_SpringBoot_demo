...
public class ChatService {
    ...
    @Autowired
    public ChatService(AssistantFactory assistantFactory, DatabaseChatMemoryStore memoryStore, ChatSessionService chatSessionService) {
    ...
    public MessageResponseVO sendMessage(String sessionId, Long userId, String message) {
        ...
        // 获取或创建会话特定的助手
        Assistant assistant = this.getOrCreateAssistant(sessionId);
    ...
    public MessageResponseVO sendMessageByUserChat(String sessionId, Long userId, String message) {
        ...
        String response = assistant.userChat(message, userId);
